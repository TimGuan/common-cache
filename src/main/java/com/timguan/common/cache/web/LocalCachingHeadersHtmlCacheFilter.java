 ...
 * 返回头中包含页面的cache信息
 *
 * @see net.sf.ehcache.constructs.web.filter.SimpleCachingHeadersPageCachingFilter
 * Created by gkq on 16/2/2.
 */
public class LocalCachingHeadersHtmlCacheFilter extends LocalHtmlCacheFilter {
        ...
        for (final Header<? extends Serializable> header : headers) {
            ...
            if ("Last-Modified".equals(header.getName())) {
                ...
                if (requestIfModifiedSince != -1) {
                    ...
                    if (!requestDate.before(pageInfoDate)) {//返回304 not modified
                        response.sendError(HttpServletResponse.SC_NOT_MODIFIED);
                        response.setHeader("Last-Modified", request.getHeader("If-Modified-Since"));
                        return;
                    }
